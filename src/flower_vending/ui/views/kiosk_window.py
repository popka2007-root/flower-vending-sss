...
class KioskMainWindow(QMainWindow):
    def __init__(
        self,
        presenter: KioskPresenter,
        *,
        window_title: str = "Flower Vending Kiosk",
        service_visible: bool = False,
    ) -> None:
        super().__init__()
        ...
        self._screen_map = {
            ...
            ScreenId.ADMIN: self._admin,
            ScreenId.ADMIN_ORDERS: self._admin,
            ScreenId.ADMIN_ANALYTICS: self._admin,
            ScreenId.ADMIN_CATALOG: self._admin,
            ScreenId.ADMIN_WINDOWS: self._admin,
            ScreenId.ADMIN_SETTINGS: self._admin,
        }
    ...
    def _wire(self) -> None:
        ...
        self._checkout.finish_requested.connect(
            lambda: (self._catalog.get_cart().clear(), self._run(self._presenter.show_home()))
        )
        self._pin.pin_accepted.connect(
            lambda pin: self._run(self._presenter.open_service_mode(pin=pin))
        )
        ...
        self._diagnostics.recover_requested.connect(
            lambda tid: self._run(self._presenter.recover_transaction(tid))
        )
    ...
    def _on_pay(self, method: str) -> None:
        cart = self._catalog.get_cart()
        self._run(
            self._presenter.checkout_cart(
                [(i.product_id, i.slot_id) for i in cart.items], cart.total_minor
            )
        )
    ...
    def render_screen(self, render: ScreenRender) -> None:
        ...
        if sid in admin_screens:
            tab_map = {
                ScreenId.ADMIN_ORDERS: self._orders_tab,
                ScreenId.ADMIN_ANALYTICS: self._analytics_tab,
                ScreenId.ADMIN_CATALOG: self._catalog_tab,
                ScreenId.ADMIN_WINDOWS: self._windows_tab,
                ScreenId.ADMIN_SETTINGS: self._settings_tab,
            }
            tab = tab_map.get(sid, self._orders_tab)
            if render.model and hasattr(tab, "bind"):
                tab.bind(render.model)
            if sid in (ScreenId.ADMIN_ORDERS, ScreenId.ADMIN) and hasattr(
                render.model, "pending_count"
            ):
                self._admin.set_pending_count(render.model.pending_count)
            self._admin.show_tab(
                (sid if sid is not ScreenId.ADMIN else ScreenId.ADMIN_ORDERS).value
            )
